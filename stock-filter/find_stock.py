...
def valid_stock(dates):
    ...
    dir_log = 'C:\\Users\\Administrator\\Desktop\\python\\stock-filter\\log\\'
    ...
    # cursor.execute('select * from stock_000001 where date=%s or date=%s order by date desc' % (today, str_yestoday))
    # value_001 = cursor.fetchall()
    # print(value_001)
    a = 0
    count = []
    #遍历所有股票
    for i in range(0,len(value_code)):
        if re.match('000',value_code[i][0]) or re.match('002',value_code[i][0]):
            ...
            try:
                ...
                #加入这两天的数据满足昨天下跌超过2%,而且今天的开盘价低于昨天的收盘价，且今天的收盘价高于昨天的开盘价，就满足阳包阴的条件
                ...
                #之前有次sql语句出错了，order by后面没有加date，每次寻找都是0只，找了半个多小时才找出来是sql语句的问题
                flog.write('%s停牌无数据,或者请查看sql语句是否正确\n'%value_code[i][0])#一般不用管，除非执行好多天的数据都为0时那可能输sql语句有问题了
    print('总共找到%d只满足条件的股票'%a)
    flog.close()
    conn.close()
    cursor.close()
    print(count)
