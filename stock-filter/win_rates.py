...
import datetime
import find_stock
#统计当天满足阳包阴所有股票，在设置的这段时间里面有没有出现过类似的行情，并且计算出如果出现过，那么那天之后的5天收益率是多少
def rate(todays):
    print(todays)
    #满足阳包阴的这些股票，以及它们之前满足的时候收益率都写到报告里面方便查看整体情况
    count,a = find_stock.valid_stock(todays)
    dir_report = 'C:\\Users\\Administrator\\Desktop\\python\\stock-filter\\report\\'
    filename = dir_report + todays +'.log'
    fp = open(filename,'w')
    fp.write('总共找到%d只满足条件的股票分别是\n%s\n'%(a,count))
            ...
            try:
                ...
                if opens2<close1 and close2>opens1 and p_change1<-2 and p_change2>9.8:
                    ...
                    if i-6>0:
                        ...
                        print('%s的%s之后5天收益率为百分之%d'%(x,dates,wins))
                        fp.write('%s在%s之后5天收益率为百分之%d\n'%(x,dates,wins))
                    else:
